...
function TemplateListIcon({ template }) {
  ...
  return (
    <div className={styles["workout-template-icon"]}>
      <h4 className={styles.templateIconHeader}>{template.templateName}</h4>
      <div className={styles["icon-buttons"]}>
        ...
        <BurgerMenu onClick={() => setIsExercisesOpen(!isExercisesOpen)} />
      </div>

      {
        <div
          className={`${styles["workout-template-icon-exercises"]} ${
            isExercisesOpen ? styles.exercisesShowing : ""
          }`}
        >
          {template.exercises.map((exercise) => (
            <div key={exercise.id}>
              <p>
                💪 {exercise.exerciseName}: {exercise.sets} Sets x{" "}
                {exercise.reps}
                reps
              </p>
            </div>
